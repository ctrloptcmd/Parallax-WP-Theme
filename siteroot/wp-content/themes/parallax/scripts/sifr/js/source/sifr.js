...
var sIFR = new function() {
  ...
  function UserAgentDetection(sIFR) {
    ...
    if(this.ieWin) {
      ...
      if(!stop && axo) this.flashVersion = this.parseVersion(/([\d,?]+)/.exec(axo.GetVariable('$version'))[1].replace(/,/g, '.'));
  ...
  function BrokenFlashIE(sIFR) {
    ...
    var fixCallbacks = active && sIFR.ua.flashVersion < sIFR.ua.parseVersion('9.0.115');
    var resetMovies  = {};
    var registry     = {};
    ...
    this.register = function(flashNode) {
      ...
      if(fixCallbacks) window[id] = flashNode;
    };
    ...
    this.cleanup = function(id, usePlaceholder) {
      ...
      registry[id] = null;
      if(fixCallbacks) window[id] = null;
      ...
      if(node.parentNode) {
        if (usePlaceholder && node.parentNode.nodeType == 1) {
          ...
          placeholder.style.width  = node.getAttribute("width") + "px";
          placeholder.style.height = node.getAttribute("height") + "px";
